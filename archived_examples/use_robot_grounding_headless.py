#!/usr/bin/env python3
"""
Example of using robot grounding calculator to place robot with feet on ground (headless version).
"""
...
def main():
    print("=== Robot Grounding Example (Headless) ===\n")
    ...
    print("Simulating 3 seconds of physics...")
    ...
    for i in range(300):  # 3 seconds
        ...
        if i % 100 == 0:
            ...
            if current_feet is not None:
                ...
                min_foot_height = current_feet[:, 2].min().item()
                max_foot_height = current_feet[:, 2].max().item()
                print(f"Time {i/100:.1f}s:")
                print(f"  Base height: {current_base:.4f}m")
                print(f"  Foot heights: {min_foot_height:.4f}m to {max_foot_height:.4f}m (avg: {avg_foot_height:.4f}m)")
            else:
    ...
    if final_foot_positions is not None:
        final_foot_heights = final_foot_positions[:, 2]
        final_avg = final_foot_heights.mean().item()
        print(f"✓ Final average foot height after 3s simulation: {final_avg:.4f}m")
        
        if abs(final_avg) < 0.05:  # Within 5cm of ground
            print("✓ Robot remained stable and grounded throughout simulation!")
        else:
            print(f"⚠ Robot drifted {final_avg*1000:.1f}mm from ground during simulation")
